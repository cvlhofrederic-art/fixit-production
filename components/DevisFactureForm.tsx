...
export default function DevisFactureForm({
  ...
  const [insuranceNumber, setInsuranceNumber] = useState(initialData?.insuranceNumber || tpl('insuranceNumber') || artisan?.insurance_number || '')
  const [insuranceName, setInsuranceName] = useState(initialData?.insuranceName || tpl('insuranceName') || artisan?.insurance_name || '')
  const [insuranceCoverage, setInsuranceCoverage] = useState(initialData?.insuranceCoverage || tpl('insuranceCoverage') || artisan?.insurance_coverage || (locale === 'pt' ? 'Portugal Continental' : 'France métropolitaine'))
  const [insuranceType, setInsuranceType] = useState<'rc_pro' | 'decennale' | 'both'>(initialData?.insuranceType || tpl('insuranceType') || artisan?.insurance_type || 'rc_pro')
  ...
  if (tvaEnabled) {
    ...
    if (!inv.ok) {
      if (process.env.NODE_ENV !== 'production') console.warn('[Artisan totaux] invariant cassé', inv)
      try {
        Sentry.captureMessage('invoice-invariant-broken-artisan', {
          level: 'warning',
          extra: { subtotalHT, totalTVA, totalTTC, delta: inv.delta },
        })
