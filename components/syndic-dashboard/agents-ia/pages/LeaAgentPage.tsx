...
type AppelCharges = { id?: string; periode: string; totalBudget: number; lots?: string; statut: string; dateCreation?: string }
type EcritureCompta = { id?: string; date: string; journal: string; libelle: string; debit: number; credit: number; compte: string; immeuble?: string }
type Budget = { id?: string; immeuble: string; annee: number; postes: Array<{ libelle?: string; nom?: string; budget?: number; montant?: number; realise?: number }> }
...
export default function LeaAgentPage({ user, immeubles = [] }: { user: UserWithProfile; immeubles?: Immeuble[] }) {
  ...
  // Comptable data — loaded from same localStorage keys used by ComptaCoproSection
  ...
  useEffect(() => {
    setLots(readLS<Lot>(`fixit_lots_${uid}`))
    setEcritures(readLS<EcritureCompta>(`fixit_journal_${uid}`))
    setAppels(readLS<AppelCharges>(`fixit_appels_${uid}`))
  ...
  const buildLeaContext = () => {
    ...
    return {
      ...
      lots: lots.map(l => ({ numero: l.numero, proprietaire: l.proprietaire, tantieme: l.tantieme, etage: l.etage, surface: l.surface })),
      totalTantiemes,
      ecritures: ecritures.slice(0, 30).map(e => ({ date: e.date, journal: e.journal, libelle: e.libelle, debit: e.debit, credit: e.credit, compte: e.compte })),
      ecrituresStats: { totalDebit, totalCredit, solde: totalCredit - totalDebit },
      appels: appels.map(a => ({ statut: a.statut, periode: a.periode, totalBudget: a.totalBudget })),
      budgets: budgets.map(b => ({ immeuble: b.immeuble, annee: b.annee, postes: b.postes })),
    }
  }
        ...
        <div className="flex gap-2">
          <textarea
            ...
            placeholder={locale === 'pt'
              ? `Faça uma pergunta sobre ${imm?.nom || 'o condomínio'}… (regulamento, encargos, AG, atrasos…)`
              : `Posez une question sur ${imm?.nom || 'la copropriété'}… (règlement, charges, AG, impayés…)`}
