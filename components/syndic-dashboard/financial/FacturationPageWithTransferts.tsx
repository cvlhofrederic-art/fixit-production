...
export default function FacturationPageWithTransferts({ missions, user, userRole, onOpenMission }: {
  ...
  const destLabels: Record<string, string> = isPt ? {
    comptable: '🧮 Contabilidade',
    valideur: '✅ Validador',
    syndic: '🏛️ Administrador',
  } : {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard emoji="💶" label={isPt ? 'Faturado (missões)' : 'Facturé (missions)'} value={`${totalFacture.toLocaleString(dateFmtLocale)} €`} sub={isPt ? `${missions.filter(m => m.montantFacture).length} faturas` : `${missions.filter(m => m.montantFacture).length} factures`} color="green" />
        <StatCard emoji="📋" label={isPt ? 'Orçamentos em curso' : 'Devis en cours'} value={`${totalDevis.toLocaleString(dateFmtLocale)} €`} sub={isPt ? `${missions.filter(m => m.montantDevis && !m.montantFacture).length} orçamentos` : `${missions.filter(m => m.montantDevis && !m.montantFacture).length} devis`} color="blue" />
        <StatCard emoji="📤" label={isPt ? 'Dossiês transferidos' : 'Dossiers transférés'} value={String(transferts.length)} sub={isPt ? `${enAttente} em espera` : `${enAttente} en attente`} color="purple" />
        <StatCard emoji="✅" label={isPt ? 'Validados contabilidade' : 'Validés comptabilité'} value={String(transferts.filter(t => t.statut === 'validé').length)} color="green" />
      </div>

      {/* Sub tabs */}
      <div className="flex gap-1 bg-[#F7F4EE] rounded-xl p-1 w-fit">
        <button onClick={() => setActiveSubTab('factures')} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${activeSubTab === 'factures' ? 'bg-white shadow text-[#0D1B2E]' : 'text-gray-500'}`}>📄 {isPt ? 'Faturas & Orçamentos' : 'Factures & Devis'}</button>
        <button onClick={() => setActiveSubTab('transferts')} className={`relative px-4 py-2 rounded-lg text-sm font-medium transition ${activeSubTab === 'transferts' ? 'bg-white shadow text-[#0D1B2E]' : 'text-gray-500'}`}>
          📤 {isPt ? 'Dossiês transferidos' : 'Dossiers transférés'}
      ...
      {activeSubTab === 'factures' && (
        <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
          <h3 className="font-bold text-[#0D1B2E] mb-4">{isPt ? 'Faturas & orçamentos das missões' : 'Factures & devis des missions'}</h3>
          <div className="space-y-2">
            {missions.filter(m => m.montantFacture || m.montantDevis).length === 0 ? (
              <div className="text-center py-8 text-gray-500">{isPt ? 'Nenhuma fatura nem orçamento nas missões' : 'Aucune facture ni devis sur les missions'}</div>
              ...
              <div key={m.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-xl hover:bg-[#F7F4EE] transition cursor-pointer" onClick={() => onOpenMission(m)}>
                <div>
                  ...
                  <p className="text-xs text-gray-500">{m.artisan} · {m.locataire ? `👤 ${m.locataire}` : ''} {m.etage ? (isPt ? `· Andar ${m.etage}` : `· Ét. ${m.etage}`) : ''}</p>
                ...
                <div className="text-right flex flex-col items-end gap-1">
                  ...
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${m.montantFacture ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>{m.montantFacture ? (isPt ? 'Faturado' : 'Facturé') : (isPt ? 'Orçamento' : 'Devis')}</span>
                  {(m as any).transfertCompta && <span className="text-xs bg-[#F7F4EE] text-[#C9A84C] px-2 py-0.5 rounded-full">📤 {isPt ? 'Transferido' : 'Transféré'}</span>}
                </div>
              </div>
            ))}
          </div>
      ...
      {activeSubTab === 'transferts' && (
        <div className="space-y-4">
          <div className="flex gap-2 items-center">
            <span className="text-sm text-gray-600">{isPt ? 'Filtrar' : 'Filtrer'} :</span>
            {(isPt ? [['', 'Todos'], ['en_attente_validation', '⏳ Em espera'], ['validé', '✅ Validados'], ['refusé', '❌ Recusados']] : [['', 'Tous'], ['en_attente_validation', '⏳ En attente'], ['validé', '✅ Validés'], ['refusé', '❌ Refusés']]).map(([val, label]) => (
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-12 text-gray-500 border-2 border-dashed border-gray-200 rounded-2xl">
              ...
              <p>{isPt ? 'Nenhum dossiê transferido por agora' : 'Aucun dossier transféré pour l\'instant'}</p>
              <p className="text-sm mt-1">{isPt ? 'Os gestores técnicos podem transferir dossiês a partir das ordens de missão' : 'Les gestionnaires techniques peuvent transférer des dossiers depuis les ordres de mission'}</p>
            </div>
            ...
            <div key={t.id} className={`bg-white rounded-2xl shadow-sm p-5 border-l-4 ${t.statut === 'en_attente_validation' ? 'border-orange-400' : t.statut === 'validé' ? 'border-green-400' : 'border-red-400'}`}>
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <span className="text-xs text-gray-500">{isPt ? 'Missão' : 'Mission'} #{t.missionId}</span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-3 mt-1 text-sm text-gray-600">
                    ...
                    {t.batiment && <span>🏢 {isPt ? 'Bloco' : 'Bât.'} {t.batiment}</span>}
                    {t.etage && <span>🏗️ {isPt ? 'Andar' : 'Ét.'} {t.etage}</span>}
                    {t.numLot && <span>🔢 {isPt ? 'Fração' : 'Lot'} {t.numLot}</span>}
                  </div>
                  ...
                  {t.note && <p className="text-xs bg-yellow-50 text-yellow-700 rounded px-2 py-1 mt-1">📝 {isPt ? 'Nota' : 'Note'} : {t.note}</p>}
                </div>
                <div className="text-right ml-4 flex-shrink-0">
                  {t.montantFacture && <p className="font-bold text-lg text-[#0D1B2E]">{t.montantFacture.toLocaleString(dateFmtLocale)} €</p>}
                  {t.montantDevis && !t.montantFacture && <p className="font-bold text-lg text-amber-700">{isPt ? 'Orçamento' : 'Devis'} {t.montantDevis.toLocaleString(dateFmtLocale)} €</p>}
                  ...
                  <p className="text-xs text-gray-500">{isPt ? 'Por' : 'Par'} : {t.transferePar}</p>
                </div>
              </div>
              ...
              {t.statut === 'en_attente_validation' && (
                <div className="flex gap-2 pt-3 border-t border-gray-100">
                  ...
                  >
                    ✅ {isPt ? 'Validar & integrar na contabilidade' : 'Valider & intégrer en comptabilité'}
                  </button>
                  <button
                    onClick={() => {
                      const raison = window.prompt(isPt ? 'Motivo da recusa?' : 'Raison du refus ?') || (isPt ? 'Informações em falta' : 'Informations manquantes')
              ...
              {t.statut === 'validé' && (
                <div className="pt-3 border-t border-gray-100">
                  <span className="text-sm text-green-600 font-medium">✅ {isPt ? 'Validado e integrado na contabilidade' : 'Validé et intégré en comptabilité'}</span>
                </div>
              )}

              {t.statut === 'refusé' && (
                <div className="pt-3 border-t border-gray-100">
                  <span className="text-sm text-red-600">❌ {isPt ? 'Recusado' : 'Refusé'} : {t.raisonRefus}</span>
                </div>
              )}
            </div>
          ))}
