  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openCreate} className="flex items-center gap-2 bg-[#0D1B2E] text-white px-4 py-2 rounded-xl font-semibold hover:bg-[#152338] transition-colors">+ Adicionar certificado</button>
      </div>
      ...
      {certs.length === 0 ? (
        <div className="bg-white border-2 border-dashed border-gray-200 rounded-xl p-16 text-center">
          ...
          <button onClick={openCreate} className="bg-[#0D1B2E] text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-[#152338] transition-colors">+ Adicionar certificado</button>
        </div>
      ) : (
              ...
              <div key={cert.id} className={`bg-white border rounded-xl p-5 hover:shadow-md transition-shadow cursor-pointer ${expirado ? 'border-red-200' : brevemente ? 'border-orange-200' : 'border-gray-200'}`} onClick={() => setSelected(s => s?.id === cert.id ? null : cert)}>
                ...
                {selected?.id === cert.id && (
                  <div className="mt-3 pt-3 border-t border-gray-100 space-y-3" onClick={e => e.stopPropagation()}>
                    ...
                    {cert.recomendacoes && <div className="bg-[#F7F4EE] rounded-xl p-3"><p className="text-xs font-semibold text-[#0D1B2E] mb-1">💡 Recomendações</p><p className="text-xs text-[#0D1B2E]">{cert.recomendacoes}</p></div>}
                  </div>
                )}
                <div className="flex gap-2 mt-3 pt-3 border-t border-gray-100" onClick={e => e.stopPropagation()}>
                  <button onClick={() => setSelected(s => s?.id === cert.id ? null : cert)} className="flex-1 text-xs bg-[#F7F4EE] text-[#C9A84C] px-3 py-1.5 rounded-lg font-medium hover:bg-[#EDE8DF] transition-colors">{selected?.id === cert.id ? '▲ Reduzir' : '▼ Detalhe'}</button>
            ...
            <div className="p-6 space-y-5">
              <section>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2"><label className="text-xs text-gray-500 mb-1 block">Nome *</label><input className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A84C]" placeholder="Edifício Sol Nascente" value={form.edificioNom || ''} onChange={e => setForm({ ...form, edificioNom: e.target.value })} /></div>
                  <div className="col-span-2"><label className="text-xs text-gray-500 mb-1 block">Morada</label><input className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A84C]" value={form.edificioMorada || ''} onChange={e => setForm({ ...form, edificioMorada: e.target.value })} /></div>
                  <div><label className="text-xs text-gray-500 mb-1 block">Frações</label><input type="number" min={1} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A84C]" value={form.nbFracoes || ''} onChange={e => setForm({ ...form, nbFracoes: parseInt(e.target.value) || 0 })} /></div>
                  <div><label className="text-xs text-gray-500 mb-1 block">N.º Certificado</label><input className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A84C]" value={form.numeroCertificado || ''} onChange={e => setForm({ ...form, numeroCertificado: e.target.value })} /></div>
                </div>
              </section>
              <section>
                ...
                <div className="grid grid-cols-2 gap-3 mt-3">
                  <div><label className="text-xs text-gray-500 mb-1 block">Consumo (kWh/m²/ano)</label><input type="number" min={0} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A84C]" value={form.consumoKWh || ''} onChange={e => setForm({ ...form, consumoKWh: parseFloat(e.target.value) || 0 })} /></div>
                  <div><label className="text-xs text-gray-500 mb-1 block">Emissões (kgCO₂/m²/ano)</label><input type="number" min={0} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A84C]" value={form.emissoesCO2 || ''} onChange={e => setForm({ ...form, emissoesCO2: parseFloat(e.target.value) || 0 })} /></div>
                </div>
              </section>
              <section>
                <h3 className="text-sm font-semibold text-gray-700 mb-3">📅 Datas</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-xs text-gray-500 mb-1 block">Data do certificado *</label><input type="date" className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A84C]" value={form.dataCertificado || ''} onChange={e => setForm({ ...form, dataCertificado: e.target.value })} /></div>
                  <div><label className="text-xs text-gray-500 mb-1 block">Data de expiração</label><input type="date" className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A84C]" value={form.dataExpiracao || ''} onChange={e => setForm({ ...form, dataExpiracao: e.target.value })} /></div>
                  <div className="col-span-2"><label className="text-xs text-gray-500 mb-1 block">Perito qualificado</label><input className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A84C]" placeholder="Nome do perito" value={form.perito || ''} onChange={e => setForm({ ...form, perito: e.target.value })} /></div>
                </div>
              </section>
              <section><label className="text-xs text-gray-500 mb-1 block">Recomendações de obras</label><textarea className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A84C]" rows={3} placeholder="Isolamento cobertura, substituição caldeira, painéis solares..." value={form.recomendacoes || ''} onChange={e => setForm({ ...form, recomendacoes: e.target.value })} /></section>
              <div className="flex gap-3">
                <button onClick={handleSave} disabled={!form.edificioNom || !form.classeSCE || !form.dataCertificado} className="flex-1 bg-[#0D1B2E] text-white py-3 rounded-xl font-semibold hover:bg-[#152338] disabled:opacity-50 transition-colors">{editing ? '💾 Guardar' : '✅ Criar certificado'}</button>
