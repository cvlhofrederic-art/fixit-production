...
export const SYNDIC_MODULES = [
  ...
  { key: 'infractions', label: 'Suivi Infractions', icon: '⚠️', description: 'Infractions au règlement · Pipeline signalement → amende · Preuves · Historique · Modèles courrier', default: false },
  { key: 'reconciliation_bancaire', label: 'Reconciliação Bancária', icon: '🏦', description: 'Reconciliação automática transações bancárias · Matching quotas · Dashboard financeiro', default: false },
  { key: 'benchmarking', label: 'Benchmarking Imóveis', icon: '📊', description: 'Comparação KPIs entre edifícios · Rankings · Percentis · Alertas outliers · Exportação', default: false },
  { key: 'chatbot_whatsapp', label: 'Chatbot WhatsApp 24/7', icon: '🤖', description: 'Chatbot IA autónomo · Resposta automática · Classificação pedidos · Criação ocorrências', default: false },
  // ── Agents IA (Plan A) ────────────────────────────────────────
  {
    key: 'fixy_agent',
    label: 'Fixy',
  ...
  // 'max_agent' retiré : Max est désormais le module 'ia' (Max Expert)
  {
    key: 'lea_agent',
    label: 'Léa',
    icon: '👩‍💼',
  ...
  {
    key: 'automation_agent',
    label: 'Tempo',
    icon: '⏱️',
    description: 'Agent automatisations & tâches récurrentes',
    default: true,
  },
] as const
