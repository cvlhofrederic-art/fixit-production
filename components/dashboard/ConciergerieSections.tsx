...
export function ProprietesConciergerieSection({ artisan }: { artisan: Artisan | null }) {
  ...
  return (
    <div className="animate-fadeIn">
      <div className="bg-white px-5 lg:px-5 py-4 border-b border-purple-500 flex justify-between items-center">
        <h1 className="text-lg font-semibold">{'🏠'} {t('proDash.conciergerie.proprietes.title')}</h1>
        <button onClick={() => setShowModal(true)} className="bg-purple-600 text-white px-4 py-2 rounded font-semibold text-sm hover:bg-purple-700 transition">{t('proDash.conciergerie.proprietes.nouvellePropriete')}</button>
      </div>
      <div className="p-4 lg:p-5">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
          <div className="bg-white p-4 rounded-md border-l-2 border-purple-500">
            <div className="text-sm text-gray-500 mb-1">{t('proDash.conciergerie.proprietes.totalProprietes')}</div>
            <div className="text-base font-semibold text-purple-600">{proprietes.length}</div>
          </div>
          <div className="bg-white p-4 rounded-md border-l-2 border-green-500">
            ...
            <div className="text-base font-semibold text-green-600">{proprietes.filter(p => p.etatMenage === 'Propre').length}</div>
          </div>
          <div className="bg-white p-4 rounded-md border-l-2 border-orange-500">
            <div className="text-sm text-gray-500 mb-1">{t('proDash.conciergerie.proprietes.aTraiter')}</div>
            <div className="text-base font-semibold text-orange-600">{proprietes.filter(p => p.etatMenage === 'À nettoyer' || p.etatMenage === 'En maintenance').length}</div>
          </div>
        </div>
        ...
        {proprietes.length === 0 ? (
          <div className="bg-white rounded-md p-12 text-center">
            ...
            <h3 className="text-base font-semibold mb-2">{t('proDash.conciergerie.proprietes.aucunePropriete')}</h3>
            <p className="text-gray-500 mb-4">{t('proDash.conciergerie.proprietes.ajoutezProprietes')}</p>
            <button onClick={() => setShowModal(true)} className="bg-purple-600 text-white px-5 py-2.5 rounded font-semibold hover:bg-purple-700 transition">{t('proDash.conciergerie.proprietes.ajouterPropriete')}</button>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {proprietes.map(p => (
              <div key={p.id} className="bg-white rounded-md p-4 border border-gray-200 hover:border-purple-200 transition">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold ${ETAT_COLORS[p.etatMenage] || 'bg-gray-100 text-gray-700'}`}>{ETAT_LABELS[p.etatMenage] || p.etatMenage}</span>
                </div>
                ...
                <select value={p.etatMenage} onChange={e => updateEtat(p.id, e.target.value)} className="w-full border border-gray-200 rounded px-3 py-2 text-sm font-semibold focus:border-purple-500 outline-none mt-2">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-md w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="p-4 border-b"><h2 className="text-base font-semibold">{'🏠'} {t('proDash.conciergerie.proprietes.nouvelleProprieteModal')}</h2></div>
            <div className="p-4 space-y-3">
              <div>
                <label className="block text-sm font-semibold mb-1">{t('proDash.conciergerie.proprietes.nomReference')}</label>
                <input value={form.nom} onChange={e => setForm({...form, nom: e.target.value})} placeholder={t('proDash.conciergerie.proprietes.nomReferencePlaceholder')} className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-semibold mb-1">{t('proDash.conciergerie.proprietes.type')}</label>
                  <select value={form.typeLogement} onChange={e => setForm({...form, typeLogement: e.target.value})} className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none">
                ...
                <div>
                  ...
                  <input type="number" value={form.nombrePieces} onChange={e => setForm({...form, nombrePieces: e.target.value})} placeholder="3" className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-semibold mb-1">{t('proDash.conciergerie.proprietes.adresse')}</label>
                <input value={form.adresse} onChange={e => setForm({...form, adresse: e.target.value})} placeholder={t('proDash.conciergerie.proprietes.adressePlaceholder')} className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-semibold mb-1">{t('proDash.conciergerie.proprietes.proprietaire')}</label>
                  <input value={form.proprietaire} onChange={e => setForm({...form, proprietaire: e.target.value})} placeholder={t('proDash.conciergerie.proprietes.proprietairePlaceholder')} className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-1">{t('proDash.conciergerie.proprietes.telephone')}</label>
                  <input value={form.telephone} onChange={e => setForm({...form, telephone: e.target.value})} placeholder="06..." className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-semibold mb-1">{t('proDash.conciergerie.proprietes.digicode')}</label>
                  <input value={form.digicode} onChange={e => setForm({...form, digicode: e.target.value})} placeholder="A1234" className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-1">{t('proDash.conciergerie.proprietes.loyer')}</label>
                  <input type="number" value={form.loyer} onChange={e => setForm({...form, loyer: e.target.value})} placeholder="0" className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-semibold mb-1">{t('proDash.conciergerie.proprietes.notesAcces')}</label>
                <textarea value={form.notesAcces} onChange={e => setForm({...form, notesAcces: e.target.value})} rows={2} placeholder={t('proDash.conciergerie.proprietes.notesAccesPlaceholder')} className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none resize-none" />
              </div>
            </div>
            <div className="p-4 border-t flex gap-3">
              <button onClick={() => setShowModal(false)} className="flex-1 py-2.5 border border-gray-200 rounded font-semibold hover:bg-gray-50 transition">{t('proDash.conciergerie.proprietes.annuler')}</button>
              <button onClick={handleSave} className="flex-1 py-2.5 bg-purple-600 text-white rounded font-semibold hover:bg-purple-700 transition">{t('proDash.conciergerie.proprietes.ajouterLaPropriete')}</button>
            </div>
          </div>
        </div>
      )}
...
export function AccesConciergerieSection({ artisan }: { artisan: Artisan | null }) {
  ...
  return (
    <div className="animate-fadeIn">
      <div className="bg-white px-5 lg:px-5 py-4 border-b border-purple-500 flex justify-between items-center">
        <h1 className="text-lg font-semibold">{'🔑'} {t('proDash.conciergerie.accesCles.title')}</h1>
        <button onClick={() => setShowModal(true)} className="bg-purple-600 text-white px-4 py-2 rounded font-semibold text-sm hover:bg-purple-700 transition">{t('proDash.conciergerie.accesCles.nouvelAcces')}</button>
      </div>
      <div className="p-4 lg:p-5">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          <div className="bg-white p-4 rounded-md border-l-2 border-green-400 text-center">
            <div className="text-base font-semibold text-green-600">{acces.filter(a => a.statut === 'Disponible').length}</div>
            <div className="text-xs text-gray-500 mt-1">{t('proDash.conciergerie.accesCles.disponibles')}</div>
          </div>
          <div className="bg-white p-4 rounded-md border-l-2 border-yellow-400 text-center">
            <div className="text-base font-semibold text-yellow-600">{acces.filter(a => a.statut === 'En prêt').length}</div>
            <div className="text-xs text-gray-500 mt-1">{t('proDash.conciergerie.accesCles.enPret')}</div>
          </div>
          <div className="bg-white p-4 rounded-md border-l-2 border-red-400 text-center">
            <div className="text-base font-semibold text-red-600">{acces.filter(a => a.statut === 'Perdu').length}</div>
            <div className="text-xs text-gray-500 mt-1">{t('proDash.conciergerie.accesCles.perdus')}</div>
          </div>
          <div className="bg-white p-4 rounded-md border-l-2 border-purple-400 text-center">
            <div className="text-base font-semibold text-purple-600">{acces.length}</div>
        ...
        {acces.length === 0 ? (
          <div className="bg-white rounded-md p-12 text-center">
            <div className="text-6xl mb-4">🔑</div>
            <h3 className="text-base font-semibold mb-2">{t('proDash.conciergerie.accesCles.aucunAcces')}</h3>
            <p className="text-gray-500 mb-4">{t('proDash.conciergerie.accesCles.gerezCles')}</p>
            <button onClick={() => setShowModal(true)} className="bg-purple-600 text-white px-5 py-2.5 rounded font-semibold hover:bg-purple-700 transition">{t('proDash.conciergerie.accesCles.ajouterAcces')}</button>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {acces.map(a => (
              <div key={a.id} className="bg-white rounded-md p-4 border border-gray-200">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <h3 className="font-semibold">{a.propriete}</h3>
                    <span className="text-sm text-purple-600">{a.typeAcces}</span>
                  </div>
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold ${STATUT_COLORS[a.statut] || ''}`}>{STATUT_LABELS[a.statut] || a.statut}</span>
                </div>
                ...
                <select value={a.statut} onChange={e => updateStatut(a.id, e.target.value)} className="w-full border border-gray-200 rounded px-3 py-1.5 text-sm font-semibold focus:border-purple-500 outline-none">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-md w-full max-w-md">
            <div className="p-4 border-b"><h2 className="text-base font-semibold">{'🔑'} {t('proDash.conciergerie.accesCles.nouvelAccesModal')}</h2></div>
            <div className="p-4 space-y-3">
              <div>
                <label className="block text-sm font-semibold mb-1">{t('proDash.conciergerie.accesCles.propriete')}</label>
                <input value={form.propriete} onChange={e => setForm({...form, propriete: e.target.value})} placeholder={t('proDash.conciergerie.accesCles.nomPropriete')} className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-semibold mb-1">{t('proDash.conciergerie.accesCles.typeAcces')}</label>
                  <select value={form.typeAcces} onChange={e => setForm({...form, typeAcces: e.target.value})} className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none">
                ...
                <div>
                  ...
                  <input value={form.code} onChange={e => setForm({...form, code: e.target.value})} placeholder="A1234 / #5" className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-semibold mb-1">{t('proDash.conciergerie.accesCles.localisation')}</label>
                <input value={form.localisation} onChange={e => setForm({...form, localisation: e.target.value})} placeholder={t('proDash.conciergerie.accesCles.localisationPlaceholder')} className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none" />
              </div>
              <div>
                <label className="block text-sm font-semibold mb-1">{t('proDash.conciergerie.accesCles.responsable')}</label>
                <input value={form.responsable} onChange={e => setForm({...form, responsable: e.target.value})} placeholder={t('proDash.conciergerie.accesCles.nomResponsable')} className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none" />
              </div>
              <div>
                <label className="block text-sm font-semibold mb-1">{t('proDash.conciergerie.accesCles.notes')}</label>
                <textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} rows={2} placeholder={t('proDash.conciergerie.accesCles.notesPlaceholder')} className="w-full border border-gray-200 rounded px-4 py-2.5 focus:border-purple-500 outline-none resize-none" />
              </div>
            </div>
            <div className="p-4 border-t flex gap-3">
              <button onClick={() => setShowModal(false)} className="flex-1 py-2.5 border border-gray-200 rounded font-semibold hover:bg-gray-50 transition">{t('proDash.conciergerie.accesCles.annuler')}</button>
              <button onClick={handleSave} className="flex-1 py-2.5 bg-purple-600 text-white rounded font-semibold hover:bg-purple-700 transition">{t('proDash.conciergerie.accesCles.enregistrer')}</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div><h2 className="text-lg font-semibold text-gray-900">{'🌐'} {t('proDash.conciergerie.channelManager.title')}</h2><p className="text-gray-500 text-sm mt-1">{t('proDash.conciergerie.channelManager.subtitle')}</p></div>
        <button onClick={() => setShowForm(true)} className="bg-blue-600 text-white px-4 py-2 rounded font-medium hover:bg-blue-700">{t('proDash.conciergerie.channelManager.nouvelleReservation')}</button>
      </div>
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-blue-50 border border-blue-200 rounded-md p-4"><div className="text-blue-600 text-sm font-medium">{t('proDash.conciergerie.channelManager.caTotal')}</div><div className="text-base font-semibold text-blue-700 mt-1">{totalCA.toLocaleString(dateLocale)} €</div></div>
        <div className="bg-orange-50 border border-orange-200 rounded-md p-4"><div className="text-orange-600 text-sm font-medium">{t('proDash.conciergerie.channelManager.commissions')}</div><div className="text-base font-semibold text-orange-700 mt-1">{totalComm.toLocaleString(dateLocale)} €</div></div>
        <div className="bg-green-50 border border-green-200 rounded-md p-4"><div className="text-green-600 text-sm font-medium">{t('proDash.conciergerie.channelManager.netPercu')}</div><div className="text-base font-semibold text-green-700 mt-1">{(totalCA - totalComm).toLocaleString(dateLocale)} €</div></div>
        <div className="bg-purple-50 border border-purple-200 rounded-md p-4"><div className="text-purple-600 text-sm font-medium">{t('proDash.conciergerie.channelManager.reservations')}</div><div className="text-base font-semibold text-purple-700 mt-1">{confirmed.length}</div></div>
      </div>
      {showForm && (
...
export function TarificationSection({ userId }: { userId: string }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div><h2 className="text-lg font-semibold text-gray-900">{'💰'} {t('proDash.conciergerie.tarification.title')}</h2><p className="text-gray-500 text-sm mt-1">{t('proDash.conciergerie.tarification.subtitle')}</p></div>
        <button onClick={() => setShowForm(true)} className="bg-blue-600 text-white px-4 py-2 rounded font-medium hover:bg-blue-700">{t('proDash.conciergerie.tarification.ajouterLogement')}</button>
      </div>
      {showForm && (
        <div className="bg-white rounded-md border p-4">
          <div className="grid grid-cols-3 gap-3">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="space-y-3">
          {tarifs.length === 0 ? <div className="text-center py-8 text-gray-500 text-sm">{t('proDash.conciergerie.tarification.aucunLogement')}</div> : tarifs.map(tr => (
            <div key={tr.id} onClick={() => setSelected(tr)} className={`bg-white rounded-md border p-4 cursor-pointer hover:border-blue-300 ${selected?.id === tr.id ? 'border-blue-500 ring-1 ring-blue-200' : ''}`}>
        ...
        <div className="col-span-2">
          {selected ? (
            <div className="bg-white rounded-md border p-4 space-y-3">
              <h3 className="font-semibold">{selected.logement}</h3>
              <div className="grid grid-cols-3 gap-3">
              ...
              <div className="border-t pt-4">
                ...
                {Object.entries(selected.prixSaison).map(([s, p]) => (
                  <div key={s} className="flex justify-between bg-yellow-50 border border-yellow-200 rounded-lg px-4 py-2 mb-2"><span className="font-medium text-sm">{s}</span><span className="font-semibold text-yellow-700">{p} {t('proDash.conciergerie.tarification.euroNuit')}</span></div>
                ))}
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div className="flex justify-between font-semibold"><span>{t('proDash.conciergerie.tarification.totalVoyageur')}</span><span className="text-blue-700">{7 * selected.prixBase + selected.menage} €</span></div>
                </div>
              </div>
            </div>
          ) : <div className="bg-gray-50 rounded-md border border-dashed border-gray-200 flex items-center justify-center h-64 text-gray-500"><div className="text-center"><div className="text-2xl mb-2">{'💰'}</div><p>{t('proDash.conciergerie.tarification.selectLogement')}</p></div></div>}
        </div>
      </div>
    </div>
  )
...
export function CheckinOutSection({ userId }: { userId: string }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div><h2 className="text-lg font-semibold text-gray-900">{'✅'} {t('proDash.conciergerie.checkinOut.title')}</h2><p className="text-gray-500 text-sm mt-1">{t('proDash.conciergerie.checkinOut.subtitle')}</p></div>
        <button onClick={() => setShowForm(true)} className="bg-blue-600 text-white px-4 py-2 rounded font-medium hover:bg-blue-700">{t('proDash.conciergerie.checkinOut.planifier')}</button>
      </div>
      <div className="grid grid-cols-4 gap-3">
        {[[t('proDash.conciergerie.checkinOut.checkinsAujourdhui'), passages.filter(p => p.date === today && p.type === 'checkin').length, 'bg-blue-50 border-blue-200 text-blue-700'], [t('proDash.conciergerie.checkinOut.checkoutsAujourdhui'), passages.filter(p => p.date === today && p.type === 'checkout').length, 'bg-orange-50 border-orange-200 text-orange-700'], [t('proDash.conciergerie.checkinOut.planifies'), passages.filter(p => p.statut === 'planifié').length, 'bg-yellow-50 border-yellow-200 text-yellow-700'], [t('proDash.conciergerie.checkinOut.effectues'), passages.filter(p => p.statut === 'effectué').length, 'bg-green-50 border-green-200 text-green-700']].map(([label, val, cls]) => (
          <div key={String(label)} className={`border rounded-md p-4 ${cls}`}><div className="text-sm font-medium">{label}</div><div className="text-base font-semibold mt-1">{val}</div></div>
        ))}
      </div>
      {showForm && (
        <div className="bg-white rounded-md border p-4">
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? <div className="text-center py-12 text-gray-500"><div className="text-2xl mb-2">{'🔑'}</div><p>{t('proDash.conciergerie.checkinOut.aucunPassage')}</p></div> : filtered.sort((a, b) => a.heure.localeCompare(b.heure)).map(p => (
          <div key={p.id} className={`bg-white rounded-md border p-4 ${p.statut === 'effectué' ? 'opacity-70' : ''}`}>
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                <div className={`text-base w-8 h-8 rounded-full flex items-center justify-center ${p.type === 'checkin' ? 'bg-blue-100' : 'bg-orange-100'}`}>{p.type === 'checkin' ? '✅' : '🚪'}</div>
                <div>
...
export function LivretAccueilSection({ userId }: { userId: string }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div><h2 className="text-lg font-semibold text-gray-900">{'📖'} {t('proDash.conciergerie.livret.title')}</h2><p className="text-gray-500 text-sm mt-1">{t('proDash.conciergerie.livret.subtitle')}</p></div>
        <button onClick={() => setShowForm(true)} className="bg-blue-600 text-white px-4 py-2 rounded font-medium hover:bg-blue-700">{t('proDash.conciergerie.livret.creerLivret')}</button>
      </div>
      {showForm && (
        <div className="bg-white rounded-md border p-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="space-y-3">
          {livrets.length === 0 ? <div className="text-center py-8 text-gray-500 text-sm">{t('proDash.conciergerie.livret.aucunLivret')}</div> : livrets.map(l => (
            <div key={l.id} onClick={() => setSelected(l)} className={`bg-white rounded-md border p-4 cursor-pointer hover:border-blue-300 ${selected?.id === l.id ? 'border-blue-500 ring-1 ring-blue-200' : ''}`}>
        ...
        <div className="col-span-2">
          {selected ? (
            <div className="bg-white rounded-md border p-4 space-y-3">
              <h3 className="font-semibold text-sm">{selected.logement}</h3>
              <div className="grid grid-cols-2 gap-3">
          ...
          ) : <div className="bg-gray-50 rounded-md border border-dashed border-gray-200 flex items-center justify-center h-64 text-gray-500"><div className="text-center"><div className="text-2xl mb-2">{'📖'}</div><p>{t('proDash.conciergerie.livret.selectionnerLivret')}</p></div></div>}
        </div>
      </div>
    </div>
  )
...
export function PlanningMenageSection({ userId }: { userId: string }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div><h2 className="text-lg font-semibold text-gray-900">{'🧹'} {t('proDash.conciergerie.planningMenage.title')}</h2><p className="text-gray-500 text-sm mt-1">{t('proDash.conciergerie.planningMenage.subtitle')}</p></div>
        <button onClick={() => setShowForm(true)} className="bg-blue-600 text-white px-4 py-2 rounded font-medium hover:bg-blue-700">{t('proDash.conciergerie.planningMenage.planifierMenage')}</button>
      </div>
      <div className="grid grid-cols-4 gap-3">
        {[[t('proDash.conciergerie.planningMenage.aFaire'), 'à_faire', 'bg-yellow-50 border-yellow-200 text-yellow-700'], [t('proDash.conciergerie.planningMenage.enCours'), 'en_cours', 'bg-blue-50 border-blue-200 text-blue-700'], [t('proDash.conciergerie.planningMenage.fait'), 'fait', 'bg-green-50 border-green-200 text-green-700'], [t('proDash.conciergerie.planningMenage.verifie'), 'vérifié', 'bg-purple-50 border-purple-200 text-purple-700']].map(([label, statut, cls]) => (
          <div key={String(statut)} className={`border rounded-md p-4 ${cls}`}><div className="text-sm font-medium">{label}</div><div className="text-base font-semibold mt-1">{taches.filter(tc => tc.statut === statut).length}</div></div>
        ))}
      </div>
      {showForm && (
        <div className="bg-white rounded-md border p-4">
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? <div className="text-center py-12 text-gray-500"><div className="text-2xl mb-2">{'🧹'}</div><p>{t('proDash.conciergerie.planningMenage.aucuneTache')}</p></div> : filtered.sort((a, b) => a.heure.localeCompare(b.heure)).map(tc => (
          <div key={tc.id} className={`bg-white rounded-md border p-4 ${tc.statut === 'vérifié' ? 'opacity-60' : ''}`}>
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
...
export function RevPARSection({ userId }: { userId: string }) {
  ...
  return (
    <div className="space-y-3">
      <div><h2 className="text-lg font-semibold text-gray-900">{'📈'} {t('proDash.conciergerie.revpar.title')}</h2><p className="text-gray-500 text-sm mt-1">{t('proDash.conciergerie.revpar.subtitle')}</p></div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          <div key={String(label)} className={`border rounded-md p-4 ${cls}`}>
            <div className="text-sm font-semibold">{label}</div>
            <div className="text-base font-semibold mt-2">{value}</div>
