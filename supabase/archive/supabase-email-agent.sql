...
-- ══════════════════════════════════════════════════════════════════════
-- 5. Colonnes réponse envoyée (ajoutées pour le flow validation/envoi)
-- ─────────────────────────────────────────────────────────────────────
ALTER TABLE syndic_emails_analysed ADD COLUMN IF NOT EXISTS response_sent TEXT;
ALTER TABLE syndic_emails_analysed ADD COLUMN IF NOT EXISTS response_sent_at TIMESTAMPTZ;
ALTER TABLE syndic_emails_analysed ADD COLUMN IF NOT EXISTS response_approved_by UUID REFERENCES auth.users(id);
-- Statut 'repondu' est désormais valide en plus de nouveau/traite/archive/mission_cree
