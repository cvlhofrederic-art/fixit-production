...
export async function GET(request: NextRequest) {
  ...
  // TSQ-10 : erreur DB gérée explicitement — un échec du SELECT ne doit pas
  // être confondu avec un échec CSRF (ligne absente = csrf_failed, inchangé).
  const { data: tokenRow, error: nonceError } = await supabaseAdmin
  ...
  if (nonceError) {
    logger.error('[email-agent/callback] Lecture oauth_nonce échouée:', nonceError)
  ...
  try {
    ...
    // 4. Stocker les tokens dans Supabase — chiffrement applicatif UNIQUEMENT
    // (colonnes access_token_enc/refresh_token_enc + expires_at, version 2).
    // Plus AUCUNE écriture des colonnes claires legacy (dépréciées, lot DB).
    // NB : le `scope` retourné par Google n'est plus stocké — pas de colonne
    // live pour le porter ; les scopes accordés sont constants (définis par la
    // route connect : gmail.readonly + userinfo.email).
    // 4a. Tokens chiffrés (upsert onConflict syndic_id)
    try {
    ...
    // 4b. Colonnes métier : email du compte connecté + clear du nonce CSRF
