  ...
  '@graph': [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 3, name: 'Espaces Verts', item: 'https://vitfix.io/fr/copropriete/espaces-verts/' },
      ],
    },
    {
      '@type': 'FAQPage',
      mainEntity: [
        {
          '@type': 'Question',
          name: 'Le débroussaillage est-il obligatoire pour les copropriétés en PACA ?',
          acceptedAnswer: { '@type': 'Answer', text: 'Oui. En région PACA, le débroussaillage est obligatoire légalement (art. L131-10 Code forestier) pour les propriétés situées à moins de 200 m d\'espaces boisés. Les copropriétés sont pleinement concernées. En cas de non-respect, la mairie peut procéder d\'office aux travaux et facturer la copropriété. Nos équipes réalisent ces débroussaillages obligatoires dans tout le 13.' },
        },
        {
          '@type': 'Question',
          name: 'Quel est le tarif d\'entretien des espaces verts pour une copropriété à Marseille ?',
          acceptedAnswer: { '@type': 'Answer', text: 'Le tarif d\'un contrat d\'entretien d\'espaces verts en copropriété à Marseille varie selon la surface et la fréquence des interventions. Comptez en moyenne 200 à 800 € par passage pour un immeuble de taille standard. Nos contrats annuels incluent 4 à 12 passages selon la saison et la croissance végétale. Devis gratuit sur visite.' },
        },
      ],
    },
  ],
...
const PRESTATIONS = [
  { icon: '✂️', title: 'Tonte et entretien pelouse', desc: 'Tonte régulière, scarification, fertilisation. Planning saisonnier adapté au climat méditerranéen.' },
  { icon: '🌳', title: 'Élagage d\'arbres', desc: 'Élagage raisonné par grimpeurs certifiés. Sécurisation des branches dangereuses au-dessus des voies et parkings.' },
  { icon: '🌿', title: 'Taille de haies', desc: 'Taille des haies et arbustes 2 à 4 fois par an. Maintien de la hauteur conforme au règlement de copropriété.' },
  { icon: '🪓', title: 'Abattage sécurisé', desc: 'Abattage d\'arbres malades ou dangereux. Gestion des autorisations mairie si nécessaire. Évacuation et broyage.' },
  { icon: '🔥', title: 'Débroussaillage anti-incendie', desc: 'Débroussaillage obligatoire PACA (art. L131-10). Certificat de réalisation fourni pour la copropriété.' },
  { icon: '♻️', title: 'Évacuation déchets verts', desc: 'Évacuation et broyage des déchets verts après chaque intervention. Déchetterie agréée, bordereau fourni.' },
]

export default function FrCoproEspacesVertsPage() {
  return (
    <div className="min-h-screen bg-warm-gray">
      ...
      <section className="relative overflow-hidden pt-16 pb-14 md:pt-20" style={{ background: 'linear-gradient(135deg, rgba(255,214,0,0.08) 0%, #fff 40%, #F5F3EF 100%)' }}>
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav aria-label="Breadcrumb" className="mb-6 text-sm text-text-muted">
            ...
            <span className="mx-2">/</span>
            <span className="text-dark font-medium">Espaces Verts & Élagage</span>
          </nav>
          ...
          <p className="text-lg text-text-muted max-w-2xl mb-6 leading-relaxed">
            Entretien des jardins, taille de haies, élagage d&apos;arbres et débroussaillage anti-incendie obligatoire PACA pour vos copropriétés à Marseille et dans le 13. Contrat syndic, facturation mensuelle.
          </p>

          {/* Alerte débroussaillage */}
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            <a
              href={`https://wa.me/${PHONE_FR.replace('+', '')}?text=${encodeURIComponent('Bonjour VITFIX, je suis syndic/gestionnaire et souhaite un devis pour l\'entretien des espaces verts et élagage de ma copropriété à Marseille.')}`}
            ...
            >
              ...
              Devis espaces verts copropriété
            </a>
            <a href={`tel:${PHONE_FR}`} className="inline-flex items-center gap-2 border-[1.5px] border-dark text-dark rounded-full font-medium px-7 py-3 text-[0.95rem] bg-transparent hover:bg-dark hover:text-white transition-all">Appeler</a>
          </div>
          <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-text-muted">
            <span className="flex items-center gap-1.5"><span className="text-yellow">✓</span> Certificat débroussaillage</span>
            <span className="flex items-center gap-1.5"><span className="text-yellow">✓</span> Grimpeurs certifiés</span>
            <span className="flex items-center gap-1.5"><span className="text-yellow">✓</span> Contrat annuel</span>
      ...
      <section className="py-14">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-display text-[clamp(1.5rem,3vw,2rem)] font-bold tracking-tight mb-8">Nos prestations espaces verts copropriété</h2>
      ...
      <section className="py-14 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-display text-xl font-bold mb-6">Espaces verts copropriété dans le 13</h2>
          <div className="flex flex-wrap gap-2">
            {FR_CITIES.map(city => (
              <Link key={city.slug} href={`/fr/services/espaces-verts-${city.slug}/`}
                className="px-4 py-2 rounded-full border border-border/50 text-sm hover:border-yellow hover:bg-yellow/5 transition-all">
                Espaces verts {city.name}
              </Link>
            ))}
          </div>
        </div>
