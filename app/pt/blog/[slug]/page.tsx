...
import { BLOG_ARTICLES, getBlogArticle, SERVICES } from '@/lib/data/seo-pages-data'

// ── Generate static pages for all PT blog articles ──
...
// ── Page Component ──
export default async function PtBlogArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const article = getBlogArticle(slug)
  if (!article) notFound()
  ...
  // Map service slugs to display info
  const serviceMap = Object.fromEntries(
    SERVICES.map(s => [s.slug, { name: s.name, icon: s.icon }])
  )
      ...
      <article className="py-12 md:py-16">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-12 p-6 bg-white rounded-2xl border border-border/50">
            <h3 className="font-display font-bold text-dark mb-4">Precisa de um profissional na sua zona?</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {['marco-de-canaveses', 'penafiel', 'amarante', 'porto', 'vila-nova-de-gaia', 'maia'].map(citySlug => (
