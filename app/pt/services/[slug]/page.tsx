...
import { getAllPageCombos, getPageCombo, SERVICES } from '@/lib/data/seo-pages-data'
import ArtisansCatalogueSection from '@/components/ArtisansCatalogueSection'
import { PHONE_PT } from '@/lib/constants'

// ── Generate all static pages (services × cities) ──
...
// ── Page Component ──
export default async function PtServiceCityPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const combo = getPageCombo(slug)
  if (!combo) notFound()
  ...
  const waText = encodeURIComponent(`Olá VITFIX, preciso de um ${service.name.toLowerCase()} em ${city.name}. Podem ajudar-me?`)
  const waUrgText = encodeURIComponent(`URGÊNCIA! Preciso de um ${service.name.toLowerCase()} imediatamente em ${city.name}. Podem intervir?`)
    ...
    '@graph': [
      {
        ...
        areaServed: {
          '@type': 'City',
          name: city.name,
          containedInPlace: { '@type': 'AdministrativeArea', name: city.distrito },
        },
  ...
  return (
    <div className="min-h-screen bg-warm-gray">
      ...
      <section className="relative overflow-hidden pt-16 pb-14 md:pt-20 md:pb-18" style={{ background: 'linear-gradient(135deg, rgba(255,214,0,0.08) 0%, #fff 40%, #F5F3EF 100%)' }}>
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-text-muted">
            <span className="flex items-center gap-1.5"><span className="text-yellow">✓</span> Profissionais certificados e de confiança</span>
            <span className="flex items-center gap-1.5"><span className="text-yellow">✓</span> Orçamento grátis sem compromisso</span>
            <span className="flex items-center gap-1.5"><span className="text-yellow">✓</span> Serviço 7 dias por semana</span>
            <span className="flex items-center gap-1.5"><span className="text-yellow">✓</span> Avaliações de clientes verificadas</span>
          </div>
        </div>
      </section>
      ...
      <section className="py-14 md:py-18 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-display text-[clamp(1.5rem,3vw,2rem)] font-bold tracking-tight mb-3">
            Problemas resolvidos em {city.name}
          </h2>
          <p className="text-text-muted mb-8 max-w-2xl">
            Os nossos profissionais resolvem os problemas mais comuns de {service.name.toLowerCase()} na região.
          </p>
            ...
            <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              <div>
                ...
                <h3 className="font-display text-xl md:text-2xl font-bold text-white mb-2">
                  Precisa de um {service.name.toLowerCase()} agora?
                </h3>
                <p className="text-white/70 max-w-lg leading-relaxed text-[0.93rem]">{urgencyText}</p>
              </div>
              <div className="flex flex-col sm:flex-row md:flex-col gap-3 flex-shrink-0">
                <a
                  href={`https://wa.me/${PHONE_PT.replace('+', '')}?text=${waUrgText}`}
                  target="_blank"
                  rel="noopener noreferrer"
      ...
      <section className="py-14 md:py-18 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-display text-[clamp(1.5rem,3vw,2rem)] font-bold tracking-tight mb-3">
            Zona de intervenção — {city.name}
          </h2>
          <p className="text-text-muted mb-6">
            Os nossos profissionais de {service.name.toLowerCase()} intervêm em {city.name} ({city.population.toLocaleString('pt-PT')} habitantes) e em todas as freguesias:
          </p>
          ...
          {nearbyCities.length > 0 && (
            <>
              <h3 className="font-display text-xl font-bold mb-4">{service.name} nos concelhos próximos</h3>
      ...
      <section className="py-14 md:py-18 bg-white">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-text-muted mb-8">
            Respostas às perguntas mais comuns sobre os nossos serviços de {service.name.toLowerCase()}.
          </p>
      ...
      <section className="py-16 md:py-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-text-muted mb-8 max-w-md mx-auto">
            Contacte-nos por WhatsApp para um orçamento gratuito. Resposta em menos de 30 minutos, profissionais verificados.
          </p>
          <a
            href={`https://wa.me/${PHONE_PT.replace('+', '')}?text=${waText}`}
            target="_blank"
          ...
          >
            ...
            Pedir orçamento grátis — WhatsApp
          </a>
        </div>
      </section>
    </div>
